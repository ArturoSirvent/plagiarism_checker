...
# Comparación semántica sin más.

Tenemos que coger el data set, limpiarlo guardar todos los textos limpios y tockenizados según el tema de steeming words y luego hacemos una comparación sencilla.
...
Creamos una función que deje el text completamente limpio de todo:
...
#se estaba poniendo de nombre el texto con las stop-wprds y es innecesario ese uso de memoria, asi
#que eliminamos los nombres.
```

Ahora que tenemos el texto sin stopwords le vamos a aplicar el steeming para quedarnos con las raices.
...
Ya tenemos todas las palabras en raices y tenemos todas las coincidencias. 
...
Por último quedaría realizar una comprobación de las coincidencias que encontramos entre los textos de prueba y los originales.
...
```{r}
comparar(corpus$Text_clean[96],corpus$Text_clean[5])
```


Ahora ya tenemos la función que nos hace la comparación solo debemos de crear una columna en el dataset con las coincidencias que encontremos con el original:


```{r}
#para que funcione bien, vamos a 
task_list<-unique(corpus$Task)

#hacemos un loop que recorra todos los registros que no son original y según su tipo de task,
#hacemos la comparación con el original de dicha clase.
...
for (i in 1:nrow(data_no_original)){
  ...
  text_origin_aux<-as.character(unlist(corpus[(corpus$Category=="original") & (corpus$Task==aux_task),"Text_clean"]))
...
for (i in 1:nrow(data_no_original)){
  data_no_original$porcentaje[i]<-100*(data_no_original$coincidencias_count[i]/data_no_original$total_words[i])
